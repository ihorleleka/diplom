...
mysql_query("DELETE FROM `posts` WHERE `event_id` = '$eventId' LIMIT 1") or die(mysql_error());

mysql_query("UPDATE `events` SET `archived` = 1 WHERE `events`.`id` = $eventId");

$event = mysql_query("SELECT * FROM `events` WHERE id = '$eventId'");
$event = mysql_fetch_assoc($event);

$html = '<div item_id="' . $event['id'] . '" class="archivedEvent row">
