...
$query = "SELECT id, name, page_id, reserved FROM categories ORDER BY id ASC" ;                
...
if ($result->num_rows > 0) {
   ...
   {
      $query1 = "SELECT id, parent_id, name, page_id, reserved FROM subcategories WHERE parent_id = '$row[id]' ORDER BY id ASC";
      $result1 = $conn->query($query1) or die(mysql_error());
      $parent1 = array();
      while($row1 = $result1->fetch_assoc())
      {
        $query2 = "SELECT id, parent_id, name, page_id, reserved FROM subcategory WHERE parent_id = '$row1[id]' ORDER BY id ASC";
